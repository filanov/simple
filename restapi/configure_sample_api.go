...
type HelloAPI interface {
	GetHello(ctx context.Context, params hello.GetHelloParams) middleware.Responder
}
...
// and the corresponding *SampleAPI instance.
// It mounts all the business logic implementers in the right routing.
func HandlerAPI(c Config) (http.Handler, *operations.SampleAPIAPI, error) {
	spec, err := loads.Analyzed(swaggerCopy(SwaggerJSON), "")
	if err != nil {
		return nil, nil, fmt.Errorf("analyze swagger: %v", err)
	}
	api := operations.NewSampleAPIAPI(spec)
	...
	api.HelloGetHelloHandler = hello.GetHelloHandlerFunc(func(params hello.GetHelloParams) middleware.Responder {
		ctx := params.HTTPRequest.Context()
		return c.HelloAPI.GetHello(ctx, params)
	})
